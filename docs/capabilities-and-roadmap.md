...
- Price-belief **clearing mechanism** (the one remaining open question): likely **both** — a simple one-sided match for **consumer retail** (shops post belief-based asks; consumers take cheapest within budget/elasticity) and a **full double auction** where two priced sides meet (**merchant↔shop wholesale**, contested scarce resources like bridge slots). Settle precisely when we spec pricing. (Double auction = both sides post prices; sort bids desc / asks asc; match top-to-top while bid≥ask at the midpoint price; leftovers rejected, which drives belief updates. Deterministic tie-break: price, then shop id.)
