...
10. **Money-supply ledger (BUILT 2026-06-25).** Every supply-changing flow categorized Faucet/Sink; monthly+end-of-advance snapshots with derived total supply, per-channel breakdown, net delta, and a conservation discrepancy; CLI `money`/`ledger` + TUI `:ledger`. Instrument-only (the trade money leaks are visible channels, not fixed). The prerequisite for sinks (taxes/upkeep) and the labor wage loop.
11. **Party Treasury & DM money injection (decided 2026-06-25).** The party are in-world money-holders: **individual member wallets + a collective purse**. Wallets change by buying/selling with shops & merchants (tracked) and by the **DM adjusting them directly** — a faucet/sink that feeds the supply ledger, so injecting gold and watching a town's economy react is *visible*. **Warn the DM before a large injection** (not automatic) to back up — reuses snapshot/branch/fork. New ledger channels (party spend/earn, DM injection/removal).
12. **Geographic money-flow / balance-of-trade (decided 2026-06-25).** Net money **in/out per place** (settlement→region→country→continent) and **net flow between settlements along routes** — the basis for a future **trade-route-arrow visualization**. This is where *transfers* (excluded from the supply ledger) are tracked, at the geographic level; also captures party gold flowing into a town.
