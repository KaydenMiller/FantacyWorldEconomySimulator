...
- **✅ Money-supply ledger BUILT + live-validated** (commit `5cf7707`). Domain + persistence (+migration) + engine emitter + CLI `money` + TUI `:money` + tests. Conservation holds (discrepancy 0 across snapshots). 335 tests pass, warning-clean. Live-validated CLI + TUI (tmux). **Finding it surfaced:** the demo world inflates ~16M/month — allowance + merchant-sale faucets dwarf the tiny merchant-purchase sink, and consumers hoard (retail transfer is small). The trade leaks are now visible channels. (This is real signal for the future "fix trade conservation" + sinks work.)
- _Next per plan:_ Production/Pricing/Trade N+1 perf cleanup (headroom), then queryability counters.
